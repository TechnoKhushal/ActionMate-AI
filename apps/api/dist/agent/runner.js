...
async function runAgent(goal, options) {
    const { accessToken, refreshToken, dryRun = true, } = options;
    const now = new Date();
...
`,
    ...
    for (let step = 0; step < MAX_STEPS; step++) {
        ...
        for (const toolCall of message.tool_calls) {
            // Our ActionMate tools are function tools.
            // OpenAI's type also allows other tool-call variants,
            // so narrow the union before accessing .function.
            ...
            try {
                ...
                try {
                    ...
                    if (toolName === "find_contact") {
                        ...
                        const contacts = await (0, contacts_1.findContact)(accessToken, refreshToken, String(args.name));
                    ...
                    // ---------------------------------------------------------
                        ...
                        const events = await (0, calendar_1.getCalendarEvents)(accessToken, refreshToken, start.toISOString(), end.toISOString());
                        result = {
                            success: true,
                            events,
                        };
                    ...
                    // ---------------------------------------------------------
                        ...
                        if (!Number.isInteger(args.windowStartHour)) {
                            throw new Error("windowStartHour must be an integer");
                        }
                        if (!Number.isInteger(args.windowEndHour)) {
                            throw new Error("windowEndHour must be an integer");
                        }
                        const slot = await (0, calendar_1.findAvailableSlot)(accessToken, refreshToken, String(args.date), args.durationMinutes, args.windowStartHour, args.windowEndHour);
                    ...
                    // ---------------------------------------------------------
                        ...
                        if (!verifiedSlot) {
                            throw new Error("A verified available slot must be found before creating a meeting");
                        }
                        if (!args.title) {
                            throw new Error("Missing meeting title");
                        }
                        if (!args.startDateTime) {
                            throw new Error("Missing startDateTime");
                        }
                        if (!args.endDateTime) {
                            throw new Error("Missing endDateTime");
                        }
                        if (!args.attendeeEmail) {
                            throw new Error("Missing attendeeEmail");
                        }
                        // Make sure the attendee email came from
                        // the real contact search.
                        const verifiedContact = knownContacts.some((contact) => contact.email.toLowerCase() ===
                            String(args.attendeeEmail).toLowerCase());
                        if (!verifiedContact) {
                            throw new Error("Attendee email was not obtained from find_contact");
                        }
                        ...
                        if (policy.decision === "DENY") {
                            throw new Error(`Policy denied this action: ${policy.reason}`);
                        }
                        if (policy.decision === "REQUIRE_APPROVAL") {
                            if (!verifiedSlot) {
                                throw new Error("A verified calendar slot is required before requesting approval");
                            }
                            if (args.startDateTime !==
                                verifiedSlot.startDateTime ||
                                args.endDateTime !==
                                    verifiedSlot.endDateTime) {
                                throw new Error("Meeting time does not match the verified available slot");
                            }
                            const approval = (0, store_1.createApproval)({
                                ownerId: options.ownerId,
                                tool: "create_meeting",
                                arguments: {
                                    title: String(args.title),
                                    startDateTime: String(verifiedSlot.startDateTime),
                                    endDateTime: String(verifiedSlot.endDateTime),
                                    attendeeEmail: String(args.attendeeEmail),
                                },
                                risk: policy.risk,
                                ...
                                context: {
                                    ...
                                    verifiedSlot: {
                                        startDateTime: verifiedSlot.startDateTime,
                                        endDateTime: verifiedSlot.endDateTime,
                                    },
                                },
                            });
                            return {
                                status: "WAITING_FOR_APPROVAL",
                                answer: "ActionMate found a verified meeting slot, but human approval is required before creating the meeting.",
                                contacts: knownContacts,
                                verifiedSlot,
                                approval: {
                                    id: approval.id,
                        ...
                        const existingEvents = await (0, calendar_1.getCalendarEvents)(accessToken, refreshToken, new Date(start.getTime() - 60_000).toISOString(), new Date(end.getTime() + 60_000).toISOString());
                        ...
                        if (overlappingEvent) {
                            ...
                            result = {
                                success: false,
                                error: "The verified time is no longer available.",
                                retryable: true,
                            };
                        }
                        // DRY RUN:
                        ...
                        // creating the Google event.
                            ...
                            result = {
                                ...
                                dryRun: true,
                                executed: false,
                                message: "Meeting creation skipped because ActionMate is running in dry-run mode.",
                                proposedMeeting: {
                                    title: args.title,
                                    startDateTime: verifiedSlot.startDateTime,
                                    endDateTime: verifiedSlot.endDateTime,
                        ...
                        // REAL EXECUTION
                            ...
                            const meeting = await (0, calendar_1.createMeeting)(accessToken, refreshToken, String(args.title), verifiedSlot.startDateTime, verifiedSlot.endDateTime, String(args.attendeeEmail));
                            result = {
                                success: true,
                                executed: true,
                                meeting,
        ...
        return {
            status: "FAILED",
            answer: "The task exceeded the maximum number of agent steps.",
            contacts: knownContacts,
            verifiedSlot,
            stepsUsed: MAX_STEPS,
        };
